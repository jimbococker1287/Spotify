...
def test_research_platform_lab_surfaces_blocked_claims_incomplete_locks_and_stale_paths(tmp_path: Path) -> None:
    ...
    assert claim_registry.iloc[0]["claim_readiness_status"] == "blocked"
    assert bool(claim_registry.iloc[0]["blocked"]) is True
    assert claim_registry.iloc[0]["supporting_artifact_freshness_status"] == "stale"
    assert str(claim_registry.iloc[0]["stale_supporting_artifact_path"]).endswith("benchmark_lock_smokebench_manifest.json")

    assert maturity_payload["claim_blocked_count"] == 1
