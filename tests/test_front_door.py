...
def _sample_outputs(output_root: Path) -> None:
    ...
    for name in ("creator.md", "scene_seed.md", "ranking.md", "scene.md", "seed.md", "creator_report_family.md"):
    ...
    _write_json(
        creator_dir / "creator_report_family.json",
        {
            ...
            "artifact_index_markdown": str((creator_dir / "creator_report_family.md").resolve()),
            "comparison_view_markdown": {
                ...
                "opportunity_lane_comparison": str((creator_dir / "ranking.md").resolve()),
            },
            "brief_view_markdown": {
                "scene_strategy_watch": str((creator_dir / "scene_seed.md").resolve()),
            },
        },
    )
...
def test_front_door_builds_landing_report(tmp_path: Path) -> None:
    ...
    assert report["hero"]["submission_status"] == "analysis_ready"
...
def test_front_door_writes_html_and_markdown(tmp_path: Path) -> None:
    ...
    assert "How The Branches Connect" in html
