...
def test_persist_to_sqlite_raises_when_spotify_history_cannot_be_written(tmp_path: Path) -> None:
    class _NonSerializable:
        pass

    db_path = tmp_path / "artifacts" / "metrics.sqlite"
    frame = pd.DataFrame({"artist_label": [_NonSerializable()]})
    histories = {
        "demo_model": SimpleNamespace(
            history={
                "artist_output_sparse_categorical_accuracy": [0.30],
                "val_artist_output_sparse_categorical_accuracy": [0.25],
                "val_artist_output_top_5": [0.50],
                "loss": [1.2],
                "val_loss": [1.3],
            }
        )
    ...
    with pytest.raises(RuntimeError, match="Unable to persist spotify_history"):
        persist_to_sqlite(
            frame,
            histories=histories,
            cpu_usage=[10.0],
