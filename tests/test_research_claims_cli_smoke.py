...
def test_research_claims_module_help_runs() -> None:
    ...
    assert "--refresh-existing" in result.stdout
...
def test_research_claims_cli_refresh_existing_repairs_paths_without_retraining(tmp_path: Path) -> None:
    output_dir = tmp_path / "outputs"
    run_dir = output_dir / "runs" / "run_refresh"
    analysis_dir = run_dir / "analysis"
    analysis_dir.mkdir(parents=True, exist_ok=True)
    ...
    for run_id, gap, drift in (
        ("run_refresh_prev_1", 0.49, 0.20),
        ("run_refresh_prev_2", 0.41, 0.19),
    ):
        prior_run_dir = output_dir / "runs" / run_id
        prior_analysis = prior_run_dir / "analysis"
        prior_analysis.mkdir(parents=True, exist_ok=True)
        _write_json(
            prior_run_dir / "run_manifest.json",
            {
                "run_id": run_id,
                "profile": "full",
                "timestamp": f"2026-05-04T12:00:0{1 if run_id.endswith('1') else 2}+00:00",
            },
        )
        _write_json(
            prior_analysis / "robustness_summary.json",
    ...
    history_dir = output_dir / "history"
    benchmark_manifest = history_dir / "benchmark_lock_refresh_manifest.json"
    _write_csv(
        history_dir / "benchmark_lock_refresh_summary.csv",
        [
            {"model_name": "retrieval_reranker", "model_type": "retrieval_reranker", "run_count": 3, "val_top1_mean": 0.24, "test_top1_mean": 0.29, "val_top1_ci95": 0.01},
            {"model_name": "extra_trees", "model_type": "classical", "run_count": 3, "val_top1_mean": 0.19, "test_top1_mean": 0.17, "val_top1_ci95": 0.01},
        ],
    ...
    _write_csv(
        history_dir / "benchmark_lock_refresh_significance.csv",
        [
            {
                "left_model": "retrieval_reranker",
                "right_model": "extra_trees",
    ...
    _write_json(
        benchmark_manifest,
        {
            "benchmark_id": "refresh",
            "comparison_ready": False,
            "summary": ["Benchmark lock is not ready yet."],
        },
    )

    research_dir = output_dir / "analysis" / "research_claims"
    ...
    _write_json(
        research_dir / "research_claims.json",
        {
            "run": {
                ...
                "profile": "full",
                "timestamp": "2026-05-05T12:00:00+00:00",
                "run_dir": "/tmp/stale/run_refresh",
                "run_dir_portable": "runs/run_refresh",
            },
            "benchmark_lock": {
                "benchmark_id": "refresh",
                "comparison_ready": False,
                "manifest_path": "/tmp/stale/benchmark_lock_refresh_manifest.json",
                "manifest_path_portable": "history/benchmark_lock_refresh_manifest.json",
            },
            "claims": [],
        },
    )
    ...
    assert result.returncode == 0
    assert "refresh_existing=true" in result.stdout
    claims_payload = json.loads((research_dir / "research_claims.json").read_text(encoding="utf-8"))
    assert claims_payload["run"]["run_dir_portable"] == "runs/run_refresh"
    candidate_claim = next(claim for claim in claims_payload["claims"] if claim["key"] == "candidate_ranking")
    assert candidate_claim["supporting_artifacts_portable"] == [
        "runs/run_refresh/run_results.json",
        "runs/run_refresh/analysis/ablation_summary.csv",
        "runs/run_refresh/analysis/backtest_significance.csv",
        "history/benchmark_lock_refresh_manifest.json",
    ]
    assert claims_payload["submission_readiness"]["status"] == "promising_but_unlocked"
