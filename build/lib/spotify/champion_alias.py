...
def preferred_serveable_model(
    ...
    *,
    ...
    if preferred_name:
        for row in result_rows:
            if str(row.get("model_name", "")).strip() != preferred_name:
                continue
            candidate = best_serveable_model([row], run_dir=run_dir)
            if candidate is not None:
                return candidate
            break
    return best_serveable_model(result_rows, run_dir=run_dir)
