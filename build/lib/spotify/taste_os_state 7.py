...
class TasteOSStateStore:
    ...
    def _migrate_legacy_if_needed(self) -> None:
        ...
        with self._connect() as conn:
            artists = memory_rows.get("artists", {}) if isinstance(memory_rows.get("artists", {}), dict) else {}
            for raw_key, raw_stats in artists.items():
                ...
                conn.execute(
                    ...
                    (
                        ...
                        artist_name,
                        int(raw_stats.get("like_count", 0) or 0),
                        int(raw_stats.get("repeat_count", 0) or 0),
                        int(raw_stats.get("skip_count", 0) or 0),
                        int(raw_stats.get("dislike_count", 0) or 0),
            ...
            for row in session_rows:
                conn.execute(
                    ...
                    (
                        ...
                        int(row.get("adaptive_replans", 0) or 0),
                        int(row.get("safe_route_steps", 0) or 0),
