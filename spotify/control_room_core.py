...
def _build_run_health_snapshot(manifest: dict[str, object]) -> dict[str, dict[str, object]]:
    ...
    safety = {
        ...
        "repeat_from_prev_new_model": str(robustness_guardrail.get("worst_model_name", "")),
        "repeat_from_prev_new_gap": _safe_float(robustness_guardrail.get("worst_gap")),
        "repeat_from_prev_new_top1": _safe_float(robustness_guardrail.get("worst_top1")),
        "repeat_from_prev_new_count": _safe_int(robustness_guardrail.get("worst_bucket_count"), default=0),
    }
...
def _manifest_ops_signal(
    ...
    *,
    ...
    promotion_status = str((snapshot.get("run", {}) if isinstance(snapshot.get("run", {}), dict) else {}).get("promotion_status", ""))
