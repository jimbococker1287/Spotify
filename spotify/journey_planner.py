from __future__ import annotations

from pathlib import Path
import csv
import json

import numpy as np

from .causal_friction import CausalSkipDecompositionArtifact
from .data import PreparedData
from .digital_twin import ListenerDigitalTwinArtifact
from .multimodal import MultimodalArtistSpace


def _write_csv(path: Path, rows: list[dict[str, object]], fieldnames: list[str]) -> Path:
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("w", newline="", encoding="utf-8") as outfile:
        writer = csv.DictWriter(outfile, fieldnames=fieldnames)
        writer.writeheader()
        for row in rows:
            writer.writerow(row)
    return path


def build_journey_plans(
    *,
    data: PreparedData,
    artist_labels: list[str],
    multimodal_space: MultimodalArtistSpace,
    digital_twin: ListenerDigitalTwinArtifact,
    output_dir: Path,
    logger,
) -> list[Path]:
    output_dir.mkdir(parents=True, exist_ok=True)
    horizon = max(4, int(__import__("os").environ.get("SPOTIFY_MOONSHOT_PLAN_HORIZON", "8")))
    beam_width = max(2, int(__import__("os").environ.get("SPOTIFY_MOONSHOT_PLAN_BEAM", "4")))

    seed_sequences = np.asarray(data.X_seq_test, dtype="int32")
    seed_contexts = np.asarray(data.X_ctx_test, dtype="float32")
    if len(seed_sequences) == 0:
        return []

    rows: list[dict[str, object]] = []
    summary_rows: list[dict[str, object]] = []
    for seed_idx in range(min(3, len(seed_sequences))):
        beams = [
            {
                "sequence": seed_sequences[seed_idx].copy(),
                "score": 0.0,
                "artists": [],
                "expected_skip": 0.0,
                "expected_end": 0.0,
            }
        ]
        context = seed_contexts[seed_idx]
        for step in range(horizon):
            next_beams: list[dict[str, object]] = []
            for beam in beams:
                last_artist = int(beam["sequence"][-1])
                transition = digital_twin.transition_matrix[last_artist]
                candidate_ids = np.argsort(transition)[::-1][: max(beam_width * 2, 8)]
                for candidate_id in candidate_ids.tolist():
                    similarity = float(multimodal_space.embeddings[last_artist] @ multimodal_space.embeddings[candidate_id])
                    novelty = float(1.0 - multimodal_space.popularity[candidate_id])
                    repeat_penalty = 1.0 if int(candidate_id) in beam["sequence"].tolist() else 0.0
                    energy_delta = abs(float(multimodal_space.energy[candidate_id] - multimodal_space.energy[last_artist]))
                    candidate_score = (
                        beam["score"]
                        + float(np.log(max(float(transition[candidate_id]), 1e-6)))
                        + 0.30 * novelty
                        + 0.20 * similarity
                        - 0.35 * repeat_penalty
                        - 0.10 * energy_delta
                    )
                    next_sequence = np.roll(np.asarray(beam["sequence"], dtype="int32"), -1)
                    next_sequence[-1] = int(candidate_id)
                    next_beams.append(
                        {
                            "sequence": next_sequence,
                            "score": candidate_score,
                            "artists": list(beam["artists"]) + [int(candidate_id)],
                            "expected_skip": beam["expected_skip"] + repeat_penalty * 0.1,
                            "expected_end": beam["expected_end"] + energy_delta * 0.02,
                        }
                    )
            beams = sorted(next_beams, key=lambda item: float(item["score"]), reverse=True)[:beam_width]

        if not beams:
            continue
        best = beams[0]
        for step, artist_id in enumerate(best["artists"], start=1):
            rows.append(
                {
                    "seed_index": seed_idx,
                    "step": step,
                    "artist_label": int(artist_id),
                    "artist_name": artist_labels[int(artist_id)],
                    "plan_score": float(best["score"]),
                    "expected_skip_penalty": float(best["expected_skip"]),
                    "expected_end_penalty": float(best["expected_end"]),
                }
            )
        summary_rows.append(
            {
                "seed_index": seed_idx,
                "planned_horizon": int(len(best["artists"])),
                "plan_score": float(best["score"]),
                "first_artist": artist_labels[int(best["artists"][0])] if best["artists"] else "",
                "last_artist": artist_labels[int(best["artists"][-1])] if best["artists"] else "",
            }
        )

    if not rows:
        return []

    plan_csv = _write_csv(
        output_dir / "journey_plans.csv",
        rows,
        ["seed_index", "step", "artist_label", "artist_name", "plan_score", "expected_skip_penalty", "expected_end_penalty"],
    )
    summary_path = output_dir / "journey_plans_summary.json"
    summary_path.write_text(json.dumps(summary_rows, indent=2), encoding="utf-8")
    logger.info("Built journey plans for %d seeds with horizon=%d", len(summary_rows), horizon)
    return [plan_csv, summary_path]
