...
def _configure_artist_top_tracks(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("--artists", type=str, default=None, help="Pipe-separated artist names to inspect.")
    parser.add_argument("--top-n", type=int, default=8, help="Number of history-derived artists to inspect.")
    parser.add_argument("--lookback-days", type=int, default=365, help="History window for deriving seed artists.")
    parser.add_argument("--track-limit", type=int, default=10, help="Maximum public top tracks per artist.")


def _configure_new_releases(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("--limit", type=int, default=20, help="Maximum public new-release albums to include.")
...
def _configure_artist_catalog_completeness(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("--artists", type=str, default=None, help="Pipe-separated artist names to audit.")
    parser.add_argument("--top-n", type=int, default=5, help="Number of history-derived artists to audit.")
    parser.add_argument(
        "--lookback-days",
        type=int,
        default=5000,
        help="History window for local listening coverage. Defaults to a near-lifetime export window.",
    )
    parser.add_argument(
        "--include-groups",
        type=str,
    ...
    parser.add_argument("--album-limit", type=int, default=20, help="Maximum public albums per artist to inspect.")
    parser.add_argument("--track-limit", type=int, default=50, help="Maximum public tracks per album to inspect.")
    parser.add_argument("--gap-limit", type=int, default=10, help="Maximum missing tracks/albums to highlight.")
...
def _configure_album_profile(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("--album", type=str, required=True, help="Spotify album URL, URI, or ID.")
    parser.add_argument("--track-limit", type=int, default=20, help="Maximum album tracks to include in markdown.")
    parser.add_argument(
        "--lookback-days",
        type=int,
        default=5000,
        help="History window for optional local listening coverage.",
    )


def _configure_playlist_intelligence(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("--playlist", type=str, required=True, help="Spotify playlist URL, URI, or ID.")
    parser.add_argument("--item-limit", type=int, default=100, help="Maximum playlist items to load.")
    parser.add_argument("--lookback-days", type=int, default=365, help="History window for taste-overlap analysis.")
    parser.add_argument("--top-n", type=int, default=20, help="Number of history-derived favorite artists to compare.")


def _configure_personal_release_radar(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("--artists", type=str, default=None, help="Pipe-separated seed artist names.")
    ...
    parser.add_argument("--related-limit", type=int, default=3, help="Related artists per seed when enabled.")
...
def _configure_summary(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("--max-reports", type=int, default=25, help="Maximum recent public-insights reports to index.")
    parser.add_argument("--category", type=str, default=None, help="Optional report category to summarize.")
...
_COMMAND_SPECS = (
    ...
    PublicInsightsCommandSpec(
        "artist-top-tracks",
        "Export public Spotify top tracks for favorite artists.",
        _configure_artist_top_tracks,
    ),
    PublicInsightsCommandSpec(
        "new-releases",
        "List public Spotify new releases for the selected market.",
        _configure_new_releases,
    ),
    ...
    PublicInsightsCommandSpec(
        ...
        "Compare favorite artists' public catalog tracks against your local listening history.",
        _configure_artist_catalog_completeness,
    ),
    ...
    PublicInsightsCommandSpec(
        "album-profile",
        "Profile one Spotify album with attribution-safe public catalog metadata.",
        _configure_album_profile,
    ),
    PublicInsightsCommandSpec(
        "playlist-intelligence",
        "Analyze playlist overlap, duplicates, and missing favorite artists.",
        _configure_playlist_intelligence,
    ),
    PublicInsightsCommandSpec(
        "personal-release-radar",
        "Build a personalized public-release radar from seed, related, and local-history artists.",
        _configure_personal_release_radar,
    ),
    PublicInsightsCommandSpec(
        "media-explorer",
    ...
    PublicInsightsCommandSpec(
        "summary",
        "Index recent public Spotify insights reports and suggest what to open next.",
        _configure_summary,
    ),
)
...
def build_public_insights_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--spotify-catalog-backend",
        choices=("urllib", "spotipy"),
        default=None,
        help="Spotify catalog transport. Defaults to SPOTIFY_PUBLIC_CATALOG_BACKEND or urllib.",
    )
    parser.add_argument(
        "--spotify-cache-mode",
        choices=("on", "off"),
        default="on",
        help="Persistent public Spotify API cache mode. Defaults to on.",
    )
    parser.add_argument(
        "--spotify-cache-ttl-hours",
        type=float,
        default=168.0,
        help="Persistent public Spotify API cache TTL in hours.",
    )
    parser.add_argument(
        "--spotify-request-interval-seconds",
        type=float,
        default=0.2,
        help="Minimum delay between live Spotify API HTTP requests.",
    )
